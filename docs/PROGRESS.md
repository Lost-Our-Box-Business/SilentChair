...
- [x] Multi-page auto-build pipeline — `plan_site()` → `generate_shared_css()` → `generate_shared_js()` → per-page `generate_page_html()` (max_tokens=16000 each) → `generate_initial_content()`. Orchestrated by `auto_build_website()` in `website_builder.py`.
- [x] Shared `styles.css` and `scripts.js` generated by dedicated LLM calls; all pages link to them via relative hrefs.
- [x] `scripts.js` includes: mobile nav toggle, sticky nav shadow, `IntersectionObserver` scroll-reveal, parallax hero, stat counter animation, FAQ accordion, dynamic section rendering via `fetchSections()`.
- [x] Dynamic content system — new `website_sections` table (migration 013); `generate_initial_content()` populates testimonials, team members, portfolio items at build time; published sites fetch live from `/public/website/{id}/sections`.
- [x] Public API — `backend/app/routers/website_public.py` with `GET /public/website/{id}/sections`; subdomain CORS middleware in `main.py` allows `*.silentchair.app` callers.
- [x] New `POST /api/website/build` endpoint in `website.py` wires frontend to `auto_build_website()`.
- [x] `buildWebsite()` added to `frontend/lib/website-api.ts`.
- [x] Onboarding website page rewritten — auto-triggers build on website creation; animated progress messages cycle while building (~60s); split-pane preview (chat left 40%, iframe right 60%) appears after build completes. Finish button in success banner.
- [x] Prompt quality standard: each page gets a dedicated LLM call with rich section-by-section guidance, specific CSS class requirements, `data-reveal` on all sections, no external image URLs, inter-page relative href links.
