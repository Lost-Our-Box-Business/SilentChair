...
export default function WebsitePage() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      await createWebsite(businessId, bizName);
      await load();
    } catch {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] gap-6 max-w-md mx-auto text-center">
        ...
        <div className="space-y-2">
          <h2 className="text-lg font-semibold">{t("noWebsiteTitle")}</h2>
          <p className="text-sm text-muted-foreground">{t("noWebsiteDesc")}</p>
        </div>
        {error && (
        ...
        <Button onClick={handleCreate} disabled={creating}>
          ...
          {creating ? t("settingUp") : t("startBuilding")}
        </Button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      ...
      <div className="flex items-center gap-3 px-4 py-2 border-b shrink-0 flex-wrap gap-y-2">
        <div className="flex items-center gap-2 mr-auto">
          ...
          {publishedUrl && (
            ...
            >
              <ExternalLink className="h-3 w-3" /> {t("live")}
            </a>
          )}
        </div>
        ...
        {published && (
          <span className="flex items-center gap-1 text-xs text-green-600">
            <CheckCircle2 className="h-3.5 w-3.5" /> {t("published")}
          </span>
        )}
      ...
      <div className="border-t px-4 py-3 shrink-0 flex items-center gap-3 flex-wrap">
        <Label className="text-xs text-muted-foreground shrink-0">{t("customDomain")}</Label>
        <Input
        ...
        <Button size="sm" variant="outline" className="h-7 text-xs" onClick={handleSaveDomain} disabled={savingDomain || !domainInput.trim()}>
          {savingDomain ? <Loader2 className="h-3 w-3 animate-spin" /> : t("save")}
        </Button>
        {domainSaved && (
          <span className="flex items-center gap-1 text-xs text-green-600">
        ...
        {website.custom_domain && (
          <p className="text-xs text-muted-foreground ml-2">
            {t("cnameHint")} <code className="font-mono">{website.subdomain}.silentchair.app</code>
          </p>
        )}
      </div>
    </div>
